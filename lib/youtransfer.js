...
var smtpWellknown = require("nodemailer-wellknown");
	...
	self.initialize = function() {
		self.settings.get(function(err, settings) {
			if(!err) {
				...
				console.log("You don't have a 'schedulerEnabled':true in your config.json file.");
			}
		});
	};
		...
		get: function(next) {
			self.settings.get(function(err, settings) {
				...
				if(settings.email.transporter === 'smtp') {
					// if(smtpWellknown(settings.email.service)) {
	...
	self.cleanup = function() {
		self.storageFactory.get(function(err, factory) {
			factory.purge(function(err, filesToDelete) {
				if(err) {
					console.log('An error occurred while removing expired file: ' + err.message);
					// self.log.error('An error occurred while removing expired files: ' + err.message);
				} else {
					console.log('The scheduled cleanup job has removed ' + filesToDelete.length + ' files.');
					...
					if(!err) {
						factory.getJSON(fields.bundle.id, function(err, bundle) {
							if(!err) {
								...
								var isMovie = _.find(fields.files, function(val) {
									return val.type.indexOf('video') > -1
								});
								if((fields.files.length === 1 && fields.files[0].type === 'application/zip') || isMovie) {
									delete fields.bundle.link;
								}
								fields.email.body = fields.email.body.replace(/\n/g, "<br />");
								res.renderTemplate("message.html", fields, function(err, body) {
									if(!err) {
										// TODO: sender to 'to'
										var email = {
											from: settings.email.sender,
											to: fields.email.to + ", " + fields.email.from,
