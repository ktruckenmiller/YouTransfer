		...
		async.map(files, function(filePath, callback) {
			process.nextTick(function() {
				if(filePath.match(/.json$/)) {
					fs.readFile(path.join(basedir, filePath), 'utf8', function(err, data) {
						if(data) {
							var context = JSON.parse(data);
							if(context.expires) {
								var expires = new Date(context.expires);
								if(Date.compare(expires, new Date()) < 0) {
									callback(null, [context.path, context.jsonPath]);
								}else {
									callback(null, [])
								}
							}else {
			...
			var filesToDelete = _.chain(results).flattenDeep().compact().value();
			del(filesToDelete, function(err) {
				if(err) {
					console.log(err);
				}
				theNext(err, _.compact(filesToDelete))
			})
