...
LocalFileStorage.prototype.upload = function(file, context, next) {
	...
	fs.mkdir(basedir, function() {
		...
		var writeFile = fs.createWriteStream(context.path);
...
LocalFileStorage.prototype.purge = function(next) {
	...
	fs.readdir(basedir, function(err, files) {
		...
		_.each(files, function(file) {
			...
			if(file.match(/.json$/)) {
				var data = fs.readFileSync(path.join(basedir, file), 'utf-8');
				var context = JSON.parse(data);
				
				if(context.expires) {
					var expires = new Date(context.expires);
					if(Date.compare(expires, new Date()) < 0) {
						filesToDelete = _.union(filesToDelete, _.compact([context.path, context.jsonPath]));
					}
				}
			}
		});
		del(filesToDelete, function(err) {
			if(typeof next === 'function') next(err, filesToDelete);
		});
	});
};
